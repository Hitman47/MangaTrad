...
def test_signage_and_standalone_sfx_are_auto_ignored() -> None:
    ...
    assert non_reviewable_reason(_block("WOOSH")) == "sfx/bruit"
    assert non_reviewable_reason(_block("CRASH!")) == "sfx/bruit"
    assert non_reviewable_reason(_block("AHHH-")) == "sfx/bruit"
    assert non_reviewable_reason(_block("AAAA")) == "sfx/bruit"
    assert non_reviewable_reason(_block("Sparkle Ie Ispa")) == "sfx/bruit"
    assert non_reviewable_reason(_block("Sparkle +s")) == "sfx/bruit"
    assert non_reviewable_reason(_block("Pira?")) == "sfx/bruit"
    assert non_reviewable_reason(_block("Bhi (beeeep)?")) == "sfx/bruit"
    assert non_reviewable_reason(_block("Gyuooooo (Whoooosh)?")) == "sfx/bruit"
    assert non_reviewable_reason(_block("SLPAAAAAN (srasssh)?")) == "sfx/bruit"
    assert non_reviewable_reason(_block("Higher. lower?")) == ""
    assert non_reviewable_reason(_block("It's Me! I'm coming in!")) == ""
