...
def _disable_external_translation_memory(monkeypatch) -> None:
...
def test_source_quality_gate_allows_draft_translation_for_readable_visual_edge_zone() -> None:
    block = OcrBlock(
        id="b",
        bbox=[0, 0, 1, 1],
        source_lang="en",
    ...
    assert any("brouillon" in warning for warning in result.warnings)
    assert any("bord du crop" in warning for warning in result.warnings)


def test_source_quality_gate_holds_tiny_visual_edge_fragments() -> None:
    block = OcrBlock(
        id="b",
        bbox=[0, 0, 1, 1],
        source_lang="en",
...
def test_argos_preflight_gate_blocks_bad_source_before_model(monkeypatch) -> None:
    _disable_external_translation_memory(monkeypatch)
...
def test_argos_preflight_gate_translates_visual_edge_as_review_draft(monkeypatch) -> None:
    _disable_external_translation_memory(monkeypatch)
    translator = ArgosTranslator()

    class FakeTranslation:
        def translate(self, text):  # type: ignore[no-untyped-def]
            return "Trad brouillon."

    monkeypatch.setattr(translator, "_translation_chain", lambda *args, **kwargs: [FakeTranslation()])
    block = OcrBlock(
        id="b",
        bbox=[0, 0, 1, 1],
        source_lang="en",
    ...
    assert block.translation_fr == "Trad brouillon."
    assert block.manual_status == "review"
    assert block.review_notes.startswith("[zone]")
