...
_COMMON_OCR_WORD_FIXES: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    (re.compile(r"\bPOMANCE\b", flags=re.IGNORECASE), "ROMANCE"),
    (re.compile(r"\bSTRANCE\b", flags=re.IGNORECASE), "STRANGE"),
    (re.compile(r"\bLONCW\b", flags=re.IGNORECASE), "LONG!!!"),
    ...
    (re.compile(r"\bHOWEver[.]?", flags=re.IGNORECASE), "However..."),
...
_CONTEXTUAL_OCR_FIXES: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    (
        re.compile(r"^THEN\s+\[\s+COULD['\u2019]?VE\s+INTRODUCED\s+YOU\s+TO\s+A\s+NOBLE\s+WHO\s+DOESN['\u2019]?T\s+have\s+SUCH\s+AN\s+AWFUL\s+REPUTATION[.]$", flags=re.IGNORECASE),
        "then I could've introduced you to a noble who doesn't have such an awful reputation.",
    ),
