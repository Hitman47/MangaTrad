...
_OCR_CORRECTIONS: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    (re.compile(r"^WO\s+Sholldnt\s+Even\s+Be\s+Here\s+To\s+Begin\s+Withi$", flags=re.IGNORECASE), "We shouldn't Even Be Here To Begin with!"),
    ...
    (re.compile(r"^AND\s+For\s+THAT\s+You\s+SHALL\s+Die\s+AJTHOUSAND\s+DEATHS!$", flags=re.IGNORECASE), "AND For THAT You SHALL Die a thousand DEATHS!"),
    (re.compile(r"^The\s+FLNERAL\s+SERVICEWW!$", flags=re.IGNORECASE), "The funeral service!!"),
    (re.compile(r"^It(?:'s|strynng)\s+(?:trying\s+)?To\s+eliminate\s+Elizabeth[.]$", flags=re.IGNORECASE), "It's trying To eliminate Elizabeth..."),
    (re.compile(r"^The\s+ANOMALY\s+Who\s+has\s+Derailed\s+The\s+plotl$", flags=re.IGNORECASE), "The ANOMALY Who has Derailed The plot!"),
    (re.compile(r"^Some-?\s*One,\s+Help\s+Mel$", flags=re.IGNORECASE), "Someone, help me!"),
    ...
    (re.compile(r"\bYoull\b", flags=re.IGNORECASE), "You'll"),
    (re.compile(r"\bthig\b", flags=re.IGNORECASE), "thing"),
    (re.compile(r"\bLke\b", flags=re.IGNORECASE), "like"),
    (re.compile(r"\bslppose\b", flags=re.IGNORECASE), "suppose"),
    (re.compile(r"\bRetupn\b", flags=re.IGNORECASE), "Return"),
    ...
    (re.compile(r"\bfaillre\b", flags=re.IGNORECASE), "failure"),
    (re.compile(r"\btme\b", flags=re.IGNORECASE), "time"),
    (re.compile(r"\bbefope\b", flags=re.IGNORECASE), "before"),
    (re.compile(r"\bwihin\b", flags=re.IGNORECASE), "within"),
    (re.compile(r"\bfln\b", flags=re.IGNORECASE), "fun"),
    (re.compile(r"\brulel\b", flags=re.IGNORECASE), "rule!"),
    (re.compile(r"\bsomel\b", flags=re.IGNORECASE), "some!"),
    ...
    (re.compile(r"\bs0\s+peal\s+coldw\b!?", flags=re.IGNORECASE), "so real cold!!!"),
    (re.compile(r"\btrlelll\b", flags=re.IGNORECASE), "true!!!"),
    (re.compile(r"\bprovosti\b", flags=re.IGNORECASE), "Provost"),
    (re.compile(r"\bplblic\b", flags=re.IGNORECASE), "public"),
    (re.compile(r"\bmisunderstandingu\b", flags=re.IGNORECASE), "misunderstanding"),
    (re.compile(r"\bcame\s+peek\b[.]?", flags=re.IGNORECASE), "came to peek...?"),
    ...
    (re.compile(r"\btheyte\b", flags=re.IGNORECASE), "they've"),
    (re.compile(r"\bluxs\b", flags=re.IGNORECASE), "Lux's"),
    (re.compile(r"\bfamly['\u2019]?s\b", flags=re.IGNORECASE), "family's"),
    (re.compile(r"\bfopget\b", flags=re.IGNORECASE), "forget"),
    (re.compile(r"\bfora\b", flags=re.IGNORECASE), "for a"),
    (re.compile(r"\beventlally\b", flags=re.IGNORECASE), "eventually"),
    (re.compile(r"\bnaprrow\b", flags=re.IGNORECASE), "narrow"),
    (re.compile(r"\bthpee\b", flags=re.IGNORECASE), "three"),
    (re.compile(r"\bremapkable\b", flags=re.IGNORECASE), "remarkable"),
    (re.compile(r"\byolever\b", flags=re.IGNORECASE), "you ever"),
    (re.compile(r"\btholghtful\b", flags=re.IGNORECASE), "thoughtful"),
    (re.compile(r"\bconsider-\s*ation\b", flags=re.IGNORECASE), "consideration"),
    (re.compile(r"\bls\s+survive\b", flags=re.IGNORECASE), "us survive"),
    (re.compile(r"\bwolldn['\u2019]?t\b", flags=re.IGNORECASE), "wouldn't"),
    (re.compile(r"\bmne\b", flags=re.IGNORECASE), "mine"),
    (re.compile(r"\bpotect\b", flags=re.IGNORECASE), "protect"),
    (re.compile(r"\basthe\b", flags=re.IGNORECASE), "as the"),
    (re.compile(r"\bfourhoy\b", flags=re.IGNORECASE), "four holy"),
    (re.compile(r"\bknghts\b", flags=re.IGNORECASE), "knights"),
    (re.compile(r"\bmediocpe\b", flags=re.IGNORECASE), "mediocre"),
    (re.compile(r"\bppovide\b", flags=re.IGNORECASE), "provide"),
    (re.compile(r"\bwonderfllly\b", flags=re.IGNORECASE), "wonderfully"),
    (re.compile(r"\b4\s+few\b", flags=re.IGNORECASE), "a few"),
    (re.compile(r"\bhowever\b", flags=re.IGNORECASE), "However..."),
    ...
    (re.compile(r"\bfor\s+stronger\s+opponents\s*$", flags=re.IGNORECASE), "For stronger opponents..."),
    (re.compile(r"\bfor\s+more\s+intense\s+battles\s*$", flags=re.IGNORECASE), "For more intense battles..."),
    (re.compile(r"\bthe\s+captain\s+and\s+vayne\s*$", flags=re.IGNORECASE), "The captain... and Vayne"),
    (re.compile(r"\bobviously,\s+sensed\s+the\s+danger\b", flags=re.IGNORECASE), "Obviously, I sensed the danger"),
    (re.compile(r"\bhe\s+'?returned\s+the\s+dam\s+age\?!", flags=re.IGNORECASE), "He returned the damage?!"),
    (re.compile(r"\bhe\s+'returned\s+the\s+damage\?!", flags=re.IGNORECASE), "He returned the damage?!"),
    ...
    (
        re.compile(r"^lore-wise,\s+he\s+might\s+be\s+able\s+to\s+stand\s+a\s+chance\s+against\s+the\s+current[.]{3}$", flags=re.IGNORECASE),
        "Lore-wise, He Might Be Able To STAND A chance AGAINST The Current Elizabeth!",
    ),
    (
        re.compile(r"^forget\s+that\s+though\s+and\s+fill\s+me\s+in\s+on\s+the$", flags=re.IGNORECASE),
        "Forget THAT Though AND Fill Me In ON The Fiefdom",
    ),
    (
...
_DIALOGUE_NORMALIZATIONS: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    (re.compile(r"^kee\s+HEE\s+hee\s+Hee\s+You['\u2019]?ll\s+Make\s+for\s+Perfect\s+TARGET\s+PRACTICE!$", flags=re.IGNORECASE), "hee hee hee... you'll make for perfect target practice!"),
    (re.compile(r"^The\s+Only\s+thing\s+a\s+WEAK,\s+Lowly\s+CAT\s+like\s+You\s+DESERVES\s+IS\s+DEATH!$", flags=re.IGNORECASE), "the only thing a weak, lowly cat like you deserves is death!"),
    (re.compile(r"^I\s+suppose\s+I\s+Should\s+Return\s+The\s+FAVOR[.]$", flags=re.IGNORECASE), "I suppose I should return the favor."),
    (re.compile(r"^Did\s+She\s+use\s+A\s+REFLECTION\s+SPELL[?!]{2}$", flags=re.IGNORECASE), "did she use a reflection spell?!"),
    ...
    (re.compile(r"^The\s+funeral\s+service!!$", flags=re.IGNORECASE), "the funeral service!!"),
    (re.compile(r"^It['â€™]?s\s+trying\s+To\s+eliminate\s+Elizabeth[.]{1,3}$", flags=re.IGNORECASE), "it's trying to eliminate Elizabeth..."),
    (re.compile(r"^The\s+ANOMALY\s+Who\s+has\s+Derailed\s+The\s+plot!$", flags=re.IGNORECASE), "the anomaly who has derailed the plot!"),
    (
        re.compile(
    ...
    (re.compile(r"^h-?hey!\s+let\s+go\s+of\s+me!$", flags=re.IGNORECASE), "hey! let go of me!"),
    (re.compile(r"^heh\s+heh\s+heh,\s+well\s+have\s+some\s+fun\s+and\s+make\s+sure\s+she\s+doesn['’]?t\s+get\s+in\s+prince\s+lux['’]?s\s+way[.]?$", flags=re.IGNORECASE), "heh heh heh, we'll have some fun and make sure she doesn't get in Prince Lux's way."),
    (re.compile(r"^they['’]?ve\s+judged\s+that\s+ms[,.:]?\s+elizabeth\s+is\s+mostly\s+innocent!$", flags=re.IGNORECASE), "they've judged that Ms. Elizabeth is mostly innocent!"),
    (re.compile(r"^lady\s+eliza-?\s*beth[,.\s]*\?$", flags=re.IGNORECASE), "Lady Elizabeth...?"),
    (re.compile(r"^lady\s+elizabeth[,.\s]*!$", flags=re.IGNORECASE), "Lady Elizabeth...!"),
    (re.compile(r"^this\s+happened\s+in\s+the\s+game,\s+too[.]$", flags=re.IGNORECASE), "this happened in the game, too."),
    (re.compile(r"^however\s+he['’]?s\s+one\s+of\s+the\s+powerful\s+seven\s+great\s+sages\s+of\s+mankind,?$", flags=re.IGNORECASE), "however, he's one of the powerful seven great sages of mankind."),
    (re.compile(r"^so,\s+be\s+sure\s+to\s+exercise\s+caution\s+and\s+restraint,\s+pleasel$", flags=re.IGNORECASE), "So, be sure to exercise caution and restraint, Elizabeth! Please!"),
    (re.compile(r"^two\s+villains\s+working\s+together\s+to\s+weather\s+through\s+the\s+world\s+out\s+there,\s+what\s+do\s+you\s+say\?$", flags=re.IGNORECASE), "two villains working together to weather through the world out there... what do you say?"),
    (re.compile(r"^all\s+right,\s+next\s+up['’]?s\s+domestic\s+affairs[.]$", flags=re.IGNORECASE), "all right, next up is domestic affairs."),
    (re.compile(r"^supe,\s+guess\s+i\s+will\s+get\s+in\s+touch\s+with\s+echigoya,\s+too[.]$", flags=re.IGNORECASE), "sure, guess I will get in touch with Echigoya, too."),
    (re.compile(r"^give\s+me\s+that!!!\s+the\s+full\s+lowdown\s+on\s+the\s+situation[.]$", flags=re.IGNORECASE), "give me the full lowdown on the situation."),
    (re.compile(r"^did\s+peter\s+call\s+for\s+a\s+girl\s+to\s+fool\s+around\s+with\s+again\?$", flags=re.IGNORECASE), "did Peter call for a girl to fool around with again?"),
    (re.compile(r"^it['’]?s\s+boring\s+to\s+be\s+with\s+such\s+an\s+idiot,\s+even\s+if\s+it['’]?s\s+for\s+my\s+family['’]?s\s+sake[.]?$", flags=re.IGNORECASE), "it's boring to be with such an idiot, even if it's for my family's sake."),
    (re.compile(r"^though,\s+that\s+he['’]?s\s+easy\s+to\s+manipulate[.]?$", flags=re.IGNORECASE), "though, that he's easy to manipulate."),
    (re.compile(r"^forget\s+that\s+though\s+and\s+fill\s+me\s+in\s+on\s+the\s+fiefdom$", flags=re.IGNORECASE), "forget that, though, and fill me in on the fiefdom."),
    ...
    (re.compile(r"^looks\s+like\s+kurt['’]?s\s+realized\s+there['’]?s\s+money\s+to\s+be\s+made\s+from\s+a\s+man['’]?s\s+desires,?$", flags=re.IGNORECASE), "looks like Kurt's realized there's money to be made from a man's desires..."),
    (re.compile(r"^you\s+ever\s+done\s+it\s+before\?$", flags=re.IGNORECASE), "hey, you ever done it before?"),
    (re.compile(r"^of\s+course\s+not,\s+wouldn['’]?t\s+dream\s+of\s+it!$", flags=re.IGNORECASE), "of course not, wouldn't dream of it!"),
    (re.compile(r"^it['’]?s\s+been\s+your\s+thoughtful\s+consideration\s+that\s+has\s+let\s+us\s+survive[.]?$", flags=re.IGNORECASE), "it's been your thoughtful consideration that has let us survive."),
    (re.compile(r"^we['’]?ll\s+ambush\s+him\s+when\s+he['’]?s\s+leaving\s+the\s+razel\s+domain[.]$", flags=re.IGNORECASE), "we'll ambush him when he's leaving the Razel domain."),
    (re.compile(r"^the\s+banfields['’]?\s+military\s+is\s+all\s+bark\s+and\s+no\s+bite,\s+right\?$", flags=re.IGNORECASE), "the Banfields' military is all bark and no bite, right?"),
    (re.compile(r"^then\s+\[\s+could['’]?ve\s+introduced\s+you\s+to\s+a\s+noble\s+who\s+doesn['’]?t\s+have\s+such\s+an\s+awful\s+reputation[.]$", flags=re.IGNORECASE), "then I could've introduced you to a noble who doesn't have such an awful reputation."),
    (re.compile(r"^wait\?\s+there['’]?\s*\$\s+some\s+kind\s+of\s+fruity\s+smell!$", flags=re.IGNORECASE), "wait! there's some kind of fruity smell...!"),
    (re.compile(r"^will\s+protect\s+the\s+saint\s+as\s+the\s+earth\s+knight[.]$", flags=re.IGNORECASE), "I will protect the saint as the earth knight..."),
    (re.compile(r"^one\s+the\s+-?four\s+holy\s+knights\s+sacred\s+cross!$", flags=re.IGNORECASE), "...one of the four holy knights of the sacred cross!"),
    (re.compile(r"^m-my\s+precious,\s+carefree\s+teatime\s+at\s+lunch[,.\s]*$", flags=re.IGNORECASE), "M-my precious, carefree teatime at lunch..."),
    (re.compile(r"^i\s+can['’]?t\s+just\s+sit\s+here\s+little\s+bonus[.]$", flags=re.IGNORECASE), "I can't just sit here enjoying this little bonus."),
    (re.compile(r"^ordinary\s+and\s+mediocre\s+through\s+and\s+through[.]$", flags=re.IGNORECASE), "ordinary and mediocre through and through."),
    (re.compile(r"^how-\s*ever[.]$", flags=re.IGNORECASE), "However..."),
    (re.compile(r"^after\s+all,\s+it['’]?s\s+my\s+policy\s+to\s+always\s+provide\s+follow-?\s*through\s+too!$", flags=re.IGNORECASE), "after all, it's my policy to always provide followthrough services, too!"),
    (re.compile(r"^why,\s+i['’]?ll\s+provide\s+you\s+with\s+a\s+wonderfully\s+talented\s+man!$", flags=re.IGNORECASE), "why, I will provide you with a wonderfully talented man!"),
    (re.compile(r"^this\s+is\s+what\s+you\s+meant\s+about\s+not\s+relying\s+my\s+sight,\s+right\?!$", flags=re.IGNORECASE), "this is what you meant about not relying on my sight, right?!"),
    (re.compile(r"^how\s+were\s+you\s+able\s+to\s+master\s+it\s+in\s+just\s+a\s+few\s+years\?$", flags=re.IGNORECASE), "how were you able to master it in just a few years?"),
    (re.compile(r"^but\s+from\s+where\?!\s+i\s+didn['’]?t\s+even\s+see\s+anything\?\s+was\s+it\s+wind\s+magic\?\s*'?$", flags=re.IGNORECASE), "but from where?! I didn't even see anything! was it wind magic?"),
    (re.compile(r"^['’]?i['’]?ll\s+make\s+that\s+girl\s+mine\s+bwoh\?\s*i$", flags=re.IGNORECASE), "I'll make that girl mine!"),
    (re.compile(r"^has\s+been\s+turned\s+into\s+some\s+den\s+of\s+decadence\s+in\s+the\s+blink\s+of\s+an\s+eyel?\s+spl,?$", flags=re.IGNORECASE), "has been turned into some den of decadence in the blink of an eye!"),
    (
...
_TRANSLATION_OVERRIDES: dict[str, str] = {
    ...
    "hee hee hee... you'll make for perfect target practice!": "Hi hi hi... tu feras une cible parfaite !",
    "the only thing a weak, lowly cat like you deserves is death!": "La seule chose qu'un chat faible et miserable comme toi merite, c'est la mort !",
    "i suppose i should return the favor": "Je suppose que je devrais te rendre la pareille.",
    "i suppose i should return the favor.": "Je suppose que je devrais te rendre la pareille.",
    "did she use a reflection spell?": "Est-ce qu'elle a utilise un sort de reflexion ?",
    "did she use a reflection spell??": "Est-ce qu'elle a utilise un sort de reflexion ?",
    "did she use a reflection spell?!": "Est-ce qu'elle a utilise un sort de reflexion ?!",
    "someone, help me!": "Quelqu'un, aidez-moi !",
    "my lung capacity": "Ma capacite pulmonaire",
    "...my lung capacity": "...Ma capacite pulmonaire",
    "shes not hurt at all!": "elle n'est pas blessee du tout!",
    "that should hurt": "ca doit faire mal...",
    "that should hurt...": "ca doit faire mal...",
    "w-wait! not even her clothes are damaged!": "Attendez! Meme ses vetements ne sont pas endommages!",
    "and there's a bunch of them, too!": "et il y en a plein, en plus !",
    "and there is a bunch of them, too!": "et il y en a plein, en plus !",
    "tch! our next foes are evil spirits!": "TCH ! Nos prochains adversaires sont des esprits malefiques !",
    "nope! big nope!": "Non ! Absolument pas !",
    ...
    "th-the provost? did she foresee all of this...?": "La Provost ?! Elle avait prevu tout ca... ?",
    "well this girl ichinose got me a commercial driver's license": "Eh bien, cette fille, Ichinose, m'a obtenu un permis de conduire professionnel.",
    "means i'm like your \"public transit\" for the day": "En gros, je suis ton \"transport public\" pour la journee.",
    "means i am like your \"public transit\" for the day": "En gros, je suis ton \"transport public\" pour la journee.",
    "means i am like your \"public transit\" for the day.": "En gros, je suis ton \"transport public\" pour la journee.",
    "you really came to peek...?": "Tu es vraiment venue jeter un oeil... ?",
    "it's a misunderstanding just now... someone was": "C'est un malentendu !! A l'instant... quelqu'un etait...",
    "it is a misunderstanding just now... someone was": "C'est un malentendu !! A l'instant... quelqu'un etait...",
    "what on earth is this person?!": "C'est quoi, cette personne ?!",
    "unbelievable!": "Incroyable !",
    "i don't know too well myself, but they're pretty much bands who play with a lot of energy!": "Je ne m'y connais pas trop, mais ce sont surtout des groupes qui jouent avec beaucoup d'energie !",
    "i guess they're a melodic hardcore or maybe loud rock band, huh?": "Je dirais que c'est du hardcore melodique, ou peut-etre un groupe de rock bruyant, hein ?",
    "so cool!": "Trop cool !",
    "hey!": "He !",
    "we shouldn't even be here to begin with!": "Nous ne devrions meme pas etre ici pour commencer !",
    "and for that you shall die a thousand deaths!": "Et pour ca, tu mourras mille fois !",
    "the funeral service!!": "Le service funebre !",
    "the funeral service!": "Le service funebre !",
    "it's trying to eliminate elizabeth.": "Il essaie d'eliminer Elizabeth.",
    "it is trying to eliminate elizabeth": "Il essaie d'eliminer Elizabeth...",
    "it's trying to eliminate elizabeth...": "Il essaie d'eliminer Elizabeth...",
    "it is trying to eliminate elizabeth...": "Il essaie d'eliminer Elizabeth...",
    "the anomaly who has derailed the plot!": "L'anomalie qui a fait derailler l'intrigue !",
    ...
    "is that so?": "Vraiment ?",
    "they're all so ...young...": "Ils sont tous si... jeunes...",
    "they are all so ...young...": "Ils sont tous si... jeunes...",
    "they're all so... young...": "Ils sont tous si... jeunes...",
    "they are all so... young...": "Ils sont tous si... jeunes...",
    "they're all so ...young": "Ils sont tous si... jeunes...",
    "they are all so ...young": "Ils sont tous si... jeunes...",
    ...
    "...fujimura": "...Fujimura...",
    "fujimura": "Fujimura",
    "it would be great if": "Ce serait super si",
    "now i have to get out before sensei catches me": "Il faut que je me tire avant que Sensei ne me surprenne...",
    "now i gotta get out before sensei catches me": "Il faut que je me tire avant que Sensei ne me surprenne...",
    ...
    "hey! let go of me!": "Hey ! Lachez-moi !",
    "heh heh heh, we'll have some fun and make sure she doesn't get in prince lux's way": "He he he, amuse-toi bien et fais en sorte qu'elle ne se mette pas en travers du chemin du prince Lux.",
    "heh heh heh, we will have some fun and make sure she doesn't get in prince lux's way": "He he he, amuse-toi bien et fais en sorte qu'elle ne se mette pas en travers du chemin du prince Lux.",
    "they've judged that ms. elizabeth is mostly innocent!": "Le tribunal a estime que Mme Elizabeth etait en grande partie innocente !",
    "lady elizabeth...?": "Lady Elizabeth...?",
    "lady elizabeth...!": "Lady Elizabeth...!",
    "this happened in the game, too": "Ca s'est aussi passe dans le jeu.",
    "however, he's one of the powerful seven great sages of mankind": "Il fait pourtant partie des sept grands sages de l'humanite.",
    "so, be sure to exercise caution and restraint, elizabeth! please!": "Alors, fais bien attention et fais preuve de moderation, Elizabeth ! S'il te plait !",
    "two villains working together to weather through the world out there... what do you say?": "Deux mechants qui s'allient pour survivre dans ce monde hostile... Qu'en penses-tu ?",
    "all right, next up is domestic affairs": "Bon, passons maintenant aux affaires interieures.",
    "sure, guess i will get in touch with echigoya, too": "Bon, je crois que je vais aussi prendre contact avec Echigoya.",
    "give me the full lowdown on the situation": "Fais-moi un compte rendu complet de la situation.",
    "did peter call for a girl to fool around with again?": "Est-ce que Peter a de nouveau invite une fille a s'amuser avec lui ?",
    "it's boring to be with such an idiot, even if it's for my family's sake": "C'est ennuyeux d'etre avec un tel idiot, meme si c'est pour le bien de ma famille.",
    "it is boring to be with... such an idiot, even if it is for my family's sake?": "C'est ENNUYEUX d'etre avec un tel idiot, meme SI C'EST POUR LE BIEN DE MA FAMILLE.",
    "though, that he's easy to manipulate": "Par contre, c'est bien qu'il soit facile a manipuler.",
    "forget that, though, and fill me in on the fiefdom": "Oublions ca, fais-moi plutot le point sur le fief.",
    "how's the settlement planning? the military build-up?": "Comment se passe la planification de l'installation ? Et le renforcement des effectifs militaires ?",
    "i am also curious why this communication is coming from... one of the razel family's resource asteroids": "Je me demande aussi pourquoi cette communication vient de... l'un des asteroides-ressources de la famille Razel.",
    "this universe is vast": "Cet univers est vaste.",
    "i must travel in search of potential...": "Je dois voyager a la recherche du potentiel...",
    "i didn't know you considered things so fully": "Je ne savais pas que tu y avais tellement reflechi.",
    "this problem isn't mine alone": "Ce probleme ne concerne pas que moi.",
    "you will entrust the way of the flash to three pupils... three remarkable students": "Vous allez confier la voie du Flash a ces trois eleves... Trois eleves exceptionnels.",
    "will i really be able... to teach a pupil of my own?": "Est-ce que je serai vraiment capable... d'enseigner a mon propre eleve ?",
    "looks like kurt's realized there's money to be made from a man's desires...": "On dirait que Kurt a compris qu'on pouvait gagner de l'argent avec les desirs des hommes...",
    "hey, you ever done it before?": "He, tu l'as deja fait ?",
    "of course not, wouldn't dream of it!": "Bien sur que non, je n'y songerais meme pas !",
    "it's been your thoughtful consideration that has let us survive": "C'est grace a votre attention bienveillante que nous avons pu survivre.",
    "we'll ambush him when he's leaving the razel domain": "Nous lui tendrons une embuscade lorsqu'il quittera le domaine de Razel.",
    "we will ambush him when he is leaving the razel domain": "Nous lui tendrons une embuscade lorsqu'il quittera le domaine de Razel.",
    "the banfields' military is all bark and no bite, right?": "L'armee des Banfield, c'est tout dans le blabla et rien dans les faits, pas vrai ?",
    "then i could've introduced you to a noble who doesn't have such an awful reputation": "Alors j'aurais pu te presenter un noble qui n'a pas une si mauvaise reputation.",
    "then i could have introduced you to a noble who doesn't have such an awful reputation": "Alors j'aurais pu te presenter un noble qui n'a pas une si mauvaise reputation.",
    "the girls say it all the time, right?": "Les filles le disent tout le temps, pas vrai ?",
    "you definitely shouldn't be using it, meral": "Tu ne devrais vraiment pas t'en servir, Meral.",
    "wait! there's some kind of fruity smell...!": "Attendez ! Il y a une odeur fruitee... !",
    "wait! there is some kind of fruity smell...!": "Attendez ! Il y a une odeur fruitee... !",
    "i will protect the saint as the earth knight": "Je protegerai la sainte en tant que chevalier de la Terre...",
    "i will protect the saint as the earth knight...": "Je protegerai la sainte en tant que chevalier de la Terre...",
    "...one of the four holy knights of the sacred cross!": "...l'un des quatre chevaliers sacres de la Sainte Croix !",
    "m-my precious, carefree teatime at lunch...": "M-mon precieux, doux moment de calme autour d'un the au dejeuner...",
    "i can't just sit here enjoying this little bonus": "Je ne peux pas rester la a profiter de ce petit bonus.",
    "i cannot just sit here enjoying this little bonus": "Je ne peux pas rester la a profiter de ce petit bonus.",
    "ordinary and mediocre through and through": "Tout a fait ordinaire et mediocre, de bout en bout.",
    "however": "Cependant...",
    "however...": "Cependant...",
    "after all, it's my policy to always provide followthrough services, too!": "Apres tout, ma politique, c'est aussi de toujours assurer un suivi !",
    "after all, it is my policy to always provide followthrough services, too!": "Apres tout, ma politique, c'est aussi de toujours assurer un suivi !",
    "why, i will provide you with a wonderfully talented man!": "Mais bien sur, je vais vous presenter un homme au talent incroyable !",
    "this is what you meant about not relying on my sight, right?!": "C'est ca que tu voulais dire quand tu disais de ne pas me fier a ma vue, c'est ca ?!",
    "how were you able to master it in just a few years?": "Comment avez-vous pu le maitriser en seulement quelques annees ?",
    "but from where?! i didn't even see anything! was it wind magic?": "Mais d'ou ?! Je n'ai rien vu ! C'etait de la magie du vent ?",
    "i'll make that girl mine!": "Je vais faire de cette fille la mienne !",
    "i will make that girl mine!": "Je vais faire de cette fille la mienne !",
    "has been turned into some den of decadence in the blink of an eye!": "C'est devenu un repaire de decadence en un clin d'oeil !",
}
